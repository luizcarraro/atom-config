...
function installMultiple(ctx, specs, options) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const pkgAddresses = (yield Promise.all(specs
            ...
            const shrinkwrapResolution = pkgShortId && dependencyShrinkwrap
                ? dependencyShrToResolution(pkgShortId, dependencyShrinkwrap, options.registry)
                : undefined;
            return yield install(spec, ctx, Object.assign({}, options, {
                pkgShortId,
                pkgId,
                resolvedDependencies: dependencyShrinkwrap &&
                    Object.assign({}, dependencyShrinkwrap.dependencies, dependencyShrinkwrap.optionalDependencies) || {},
...
function dependencyShrToResolution(pkgShortId, depShr, registry) {
...
function install(spec, ctx, options) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!proceed && options.pkgId &&
            ...
            (yield exists(path.join(options.nodeModules, `.${options.pkgId}`)))) {
            return null;
        }
        ...
        if (fetchedPkg.isLocal) {
            ...
            logInstallStatus_1.default({ status: 'downloaded_manifest', pkgId: fetchedPkg.id, pkgVersion: fetchedPkg.pkg.version });
            return null;
        }
        ...
        let pkg;
        try {
            pkg = yield fetchedPkg.fetchingPkg;
        }
        catch (err) {
        ...
        const currentIsInstallable = (options.force ||
        ...
        if (!ctx.installs[fetchedPkg.id]) {
            ...
            ctx.installs[fetchedPkg.id] = {
                ...
                localLocation: path.join(options.nodeModules, `.${package_store_1.pkgIdToFilename(fetchedPkg.id)}`),
            };
            const children = yield installDependencies(pkg, spec, fetchedPkg.id, ctx, Object.assign({}, options, {
...
function installDependencies(pkg, parentSpec, pkgId, ctx, opts) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        return yield installMultiple(ctx, deps, depsInstallOpts);
    });
}
