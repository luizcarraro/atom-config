...
const scriptLogger = logger_1.default('run_script');
function runScript(command, args, opts) {
    ...
    return new Promise((resolve, reject) => {
        ...
        byline(proc.stdout).on('data', (line) => loggers_1.lifecycleLogger.info({
            script: scriptName,
            line: line.toString(),
            pkgId: opts.pkgId,
        }));
        byline(proc.stderr).on('data', (line) => loggers_1.lifecycleLogger.error({
            script: scriptName,
            line: line.toString(),
            pkgId: opts.pkgId,
        }));
        proc.on('close', (code) => {
            if (code > 0) {
                loggers_1.lifecycleLogger.error({
                    pkgId: opts.pkgId,
                    script: scriptName,
                    exitCode: code,
                });
            ...
            loggers_1.lifecycleLogger.info({
                pkgId: opts.pkgId,
                script: scriptName,
                exitCode: code,
            });
