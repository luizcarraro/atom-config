...
function default_1(streamParser, opts) {
    if (opts.cmd === 'server') {
        const log$ = most.fromEvent('data', streamParser);
        reporterForServer_1.default(log$);
        return;
    }
    const width = opts.width || process.stdout.columns && process.stdout.columns - 2 || 80;
    const output$ = toOutput$(streamParser, Object.assign({}, opts, { width }));
    if (opts.appendOnly) {
        output$
            .subscribe({
            complete() { },
...
function toOutput$(streamParser, opts) {
    opts = opts || {};
        ...
        streamParser['on']('data', (log) => {
            switch (log.name) {
                ...
                case 'pnpm:package-json':
                    ...
                    break;
                case 'pnpm:link':// tslint:disable-line
                    linkPushStream.next(log);
                    break;
                case 'pnpm:cli':// tslint:disable-line
    ...
    const outputs = reporterForClient_1.default(log$, isRecursive, opts.cmd, opts.width, opts.appendOnly, opts.throttleProgress);
    if (opts.appendOnly) {
